...
float generarPrecioAleatorio(float min, float max) {
    float scale = rand() / (float) RAND_MAX; // generamos un valor entre 0 y 1
...
int main() {
    srand(time(NULL)); // inicializamos la semilla para generar números aleatorios diferentes en cada ejecución
    ...
    // Ejemplo de uso de la función calcularCosto()
    struct Producto productoEjemplo = clientes[0].Productos[0];
    float costoProducto = calcularCosto(productoEjemplo);
    printf("El costo del producto %d es: %.2f\n", productoEjemplo.ProductoID, costoProducto);
